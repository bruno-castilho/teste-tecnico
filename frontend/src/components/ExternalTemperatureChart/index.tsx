...
interface ExternalTemperatureChartProps {
  open: boolean;
  onClose: () => void;
  devEui: string;
}

export function ExternalTemperatureChart({
  open,
  onClose,
  devEui,
}: ExternalTemperatureChartProps) {
  ...
  data?.deviceReadings.forEach((deviceReading) => {
    temperatures.push(deviceReading.object.Ext_TempC_SHT);
